...
Added separate **Filter** and **Search** panels. The Filter panel now supports live payload byte filtering by contiguous data sequences such as `3E 80` and `22 F1 90`. The Search panel locates the first matching row in the current filtered and sorted table without changing the visible dataset.
